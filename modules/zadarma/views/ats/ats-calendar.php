...
$phoneList = $model->getDayAtsList($month, $year);
$monthPhone= $model->getMonthAtsList($year);
...
function setMonth(m){
 document.location.href="index.php?r=/zadarma/ats/ats-calendar&noframe=1&month="+m+"&year=<?= $year ?>";   
}
</script> 
<div align='center'>
<form name='fltForm' id='fltForm' method='get' action='index.php'>
<input type='hidden' name='r' value='/zadarma/ats/ats-calendar'>
...
<tr><td>
 ...
 <?php
   ...
   $action="setMonth(".$m.");";
    echo  \yii\helpers\Html::tag( 'div', $monthPhone[$m]['all'], 
          [
                     'class'   => 'clickable',
                     'onclick' => $action,
                     'title'   => 'Входящих вонков за '.$m." месяц",
                     'style'   => $style,
          ]);    
   ...
   $action="setMonth(".$m.");";
    echo  \yii\helpers\Html::tag( 'div', $monthPhone[$m]['err'], 
          [
                     'class'   => 'clickable',
                     'onclick' => $action,
                     'title'   => 'Ошибок + не отвечено за '.$m." месяц",
                     'style'   => $style,
          ]);    
...
<table class='table table-small table-striped'>
<?php
function printCell($d, $month, $year, $phoneList)
{
   ...
   $action =  "setCalendarFilter(".$dd.",".$mm.",".$year.");";                    
    ...
    echo "<td align='left'>";
    echo  \yii\helpers\Html::tag( 'div', $phoneList[$d]['in']."&nbsp;/&nbsp;".$phoneList[$d]['out'], 
          [
                     'class'   => 'clickable',
                     'onclick' => $action,
                     'title'   => 'Всего за '.$dd." число Входящих/исходящих",
          ]);    
    echo "</td>";        
    
    echo "<td align='left'>";
       if (($phoneList[$d]['err']+$phoneList[$d]['cancel']) > 0) $style="color:Crimson";
                      else   $style="color:Green";

    echo  \yii\helpers\Html::tag( 'div', ($phoneList[$d]['err']+$phoneList[$d]['cancel']), 
          [
                     'class'   => 'clickable',
                     'onclick' => $action,
                     'title'   => 'Ошибок / не отвечено за '.$dd." число",
                     'style'   => $style,
          ]);    
    echo "</td>";        
}
   ...
   for ($i=1;$i<=$k;$i++){
    ...
    printCell($d, $month, $year, $phoneList );       
