...
/**
 * BuhMonthStatisics - заполнение бухгалтерской статистики за месяц
  ALTER TABLE `rik_buh_stat_header` ADD COLUMN `isMonth` TINYINT DEFAULT 0;
  ALTER TABLE `rik_buh_statistics`  ADD COLUMN `isMonth` TINYINT DEFAULT 0;
*/
 
 
 class BuhMonthStatistics extends Model
{
    public $dataArray = [];
    public $controlArray = [];    
    public $dateList = [];
 ...
 */
 ...
 /********************/
 /*Возвращает исполнение расписания на заданное время - для месячной это сомнительно*/
 ...
 //Получаем исполнение на указанную дату  
  $strSql = "SELECT max(id) from {{%buh_stat_header}} where DATE(checkDate) = :checkdate  and isMonth =1 ORDER BY id";   
...
/*
Задаем начальную точку  dtstart - на начало требуемого года

*/   
   if (empty($this->dtstart)) $this->dtstart=date("Y-01-01", time());  //первое января текущего года 
  
   $st_time = strtotime($this->dtstart);               
   // 0 - конец предыдущего года
   for ($i=0;$i<12;$i++ )
    {
     $this->timeList[$i+1] = strtotime(date("Y-m-01", $st_time+24*3600*31*$i ));   
     $this->dateList[$i+1] = date("d.m.Y",$this->timeList[$i+1]);          
    }     
   $lastDt =Yii::$app->db->createCommand('SELECT max(checkDate) FROM {{%buh_stat_header}} WHERE 
   isSynced=1 and checkDate <= :checkdate and isMonth =1',
 ...
 public function fillAutoBuhStatData($dateTime)
{
  $checkdate = date("Y-m-d", $dateTime);
     ...
     $record->row = 1;
     $record->headerRef= $header->id; 
     $record->auto =  Yii::$app->db->createCommand( 'SELECT  SUM(schetSumm) FROM {{%schet}} 
     WHERE YEAR(schetDate) = YEAR(:checkdate) and MONTH(schetDate) = MONTH(:checkdate)',
     ...
     $record->row = 2;
     $record->headerRef= $header->id; 
     $record->auto =  Yii::$app->db->createCommand( 'SELECT  SUM(supplySumm) FROM {{%supply}} 
     WHERE YEAR(supplyDate) = YEAR(:checkdate) and MONTH(supplyDate) = MONTH(:checkdate)',
            [ ':checkdate' =>  $checkdate  ] )->queryScalar();
     if (empty($record->auto)) $record->auto = 0;   
     $record->save();
     ...
     //Прибыль
     $headerRef = Yii::$app->db->createCommand( 'SELECT  MAX(id) FROM {{%profit_header}} WHERE 
     YEAR(onDate) = YEAR(:checkdate) and MONTH(onDate) = MONTH(:checkdate)',
             [ ':checkdate' => $checkdate  ] )->queryScalar();            
     ...
     $record->row = 4;
     $record->headerRef= $header->id; 
     $record->auto =  Yii::$app->db->createCommand( 'SELECT  SUM(oplateSumm) FROM {{%oplata}} WHERE
     YEAR(oplateDate) = YEAR(:checkdate) and MONTH(oplateDate) = MONTH(:checkdate)',
            [ ':checkdate' =>  $checkdate  ] )->queryScalar();
     if (empty($record->auto)) $record->auto = 0;   
     $record->save();
     ...
     $record->row = 5;
     $record->headerRef= $header->id; 
     $record->auto =  Yii::$app->db->createCommand('SELECT  SUM(oplateSumm) FROM {{%supplier_oplata}} WHERE
     YEAR(oplateDate) = YEAR(:checkdate) and MONTH(oplateDate) = MONTH(:checkdate)',
            [ ':checkdate' =>  $checkdate  ] )->queryScalar();
     if (empty($record->auto)) $record->auto = 0;   
     $record->save();
     ...
     //На счету    
     $headerRef = Yii::$app->db->createCommand( 'SELECT  MAX(id) FROM {{%control_bank_header}} WHERE 
     YEAR(onDate) = YEAR(:checkdate) and MONTH(onDate) = MONTH(:checkdate)',
            [ ':checkdate' => $checkdate  ] )->queryScalar();            
    ...
    //Приход товара    
    $headerRef = Yii::$app->db->createCommand( 'SELECT  MAX(id) FROM {{%control_purch_header}} WHERE 
    YEAR(onDate) = YEAR(:checkdate) and MONTH(onDate) = MONTH(:checkdate)',
            [ ':checkdate' => $checkdate  ] )->queryScalar();            
    ...
    //Склад    
    $headerRef = Yii::$app->db->createCommand( 'SELECT  MAX(id) FROM {{%ware_header}} WHERE 
    YEAR(onDate) = YEAR(:checkdate) and MONTH(onDate) = MONTH(:checkdate)',
            [ ':checkdate' => $checkdate ] )->queryScalar();            
     ...
     //долги
     $headerRef =  Yii::$app->db->createCommand('SELECT MAX(id) FROM {{%control_sverka_header}} WHERE
     YEAR(onDate) = YEAR(:checkdate) and  MONTH(onDate) = MONTH(:checkdate)', 
            [ ':checkdate' => $checkdate, ])->queryScalar();  
     if (empty ($headerRef)) $headerRef = 0;
