    ...
    {    
     ...
     {                
            ...
            $record = TblDocuments::findOne([
                'docIntNum' => mb_substr($parse[0],0,20,'utf-8')
            ]);            
            if (empty($record)) $record = new TblDocuments(); //если не нашли то новый
           ...
           //если есть хозяин первички то оплачиваемый
            ...
            $record -> contagentAccount  = mb_substr($parse[17],0,6,'utf-8');   //` VARCHAR(10) COLLATE utf8_general_ci DEFAULT '000000' COMMENT 'последние 6 знаков',
            $l=6-mb_strlen($record->contagentAccount,'utf-8');
            if($l > 0)
                $record -> contagentAccount = mb_substr("000000",0,$l,'utf-8').$record -> contagentAccount;
