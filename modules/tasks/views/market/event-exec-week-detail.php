...
function changeShowDate()
{
  ...
  document.location.href='index.php?r=/tasks/market/event-exec-week-detail&id=<?= $userId ?>&noframe=1&date='+showDate ;
}
...
<div class ='col-md-3'>   
  ...
  <div class ='col-md-2'>   
  <a href="index.php?r=tasks/market/event-exec-week&id=<?= $userId ?>&date=<?= date("Y-m-d",$now) ?>" >Кратко</a>   
  </div>
   <div class ='col-md-1'>   
       <a href="index.php?r=tasks/market/event-exec-week-detail&id=<?= $userId ?>&date=<?= date("Y-m-d",$prev) ?>" ><span class='glyphicon glyphicon-backward'></span></a>   
   </div>
...
?>      
   ...
   <div class ='col-md-1' style='text-align:right'>    
       <a href="index.php?r=tasks/market/event-exec-week-detail&id=<?= $userId ?>&date=<?= date("Y-m-d",$next) ?>" ><span class='glyphicon glyphicon-forward'></span></a>   
   </div>
  <div class ='col-md-2'>   
...
for ($iRow=0;$iRow<$model ->pN;$iRow++)
   ...
   if ($iRow%2 == 0) $bg="WhiteSmoke;";
    ...
    $add ="";
    if (!empty ($timeList['orgRef'])) 
    {        
      if ($timeList['eventStatus'] == 2) { $bgs ='LawnGreen'; 
     
         if ($timeList['creatorRef'] != $timeList['executorRef'])  $add = "<div class='close-box'><span class='glyphicon glyphicon-alert' style='color:Yellow;'></span></div>";
         if ($timeList['taskPriority'] == 1)  $add = "<div class='close-box'><span class='glyphicon glyphicon-alert' style='color:DarkTurquoise;'></span></div>";
         if ($timeList['taskPriority'] == 2)  $add = "<div class='close-box'><span class='glyphicon glyphicon-alert' style='color:Coral;'></span></div>";
      }
      else {
         $bgs=$bg; 
      ...
      onclick='openWin(\"site/contacts-detail&id=".$timeList['orgRef']."\", \"childwin\")' >".$timeList['orgTitle']."</div> ".$add."</td>";       
