...
{
    openWin("site/contacts-detail&id=" +orgRef, "childwin");
}
...
<table width='100%' border='1px' style='border-color:Gray;'>
  ...
  <tr>
    ...
    <th style='width:50px'> Звонок </th>
    <th style='width:50px'> Документ </th>    
       ...
       //if ($hN%2 == 0) $bg="WhiteSmoke;"; else 
        ...
        {                
            ...
            if ($timeList['orgRef'] > 0)
                $strOutPut .= "<td style='background-color:$bg'><a href=# onclick='showOrg(".$timeList['orgRef'].");' >".$timeList['taskTitle']."</a></td>";
            else
               ...
               $id = "isDoc".$timeList['eventRef'];                       
               $action = "switchData(".$timeList['eventRef'].", 'isDoc');";                                  
               $val = \yii\helpers\Html::tag( 'div', '&nbsp;', 
                   [
                     'class'   => 'btn btn-primary btn-small',
                     'id'      => $id,
            ...
            //Выполнено
            if ($timeList['eventStatus'] == 2) $style= 'background-color:LightGray';
            else                               $style= 'background-color:White';
            /*{          
              $time = strtotime($timeList['executeDateTime']);
              if($time > 100)  $strOutPut .="<td align='center' style='color:Green;'><span class='glyphicon glyphicon-ok' title='".date("d.m H:i",$time)."'></span></td>";                 
              else             $strOutPut .="<td align='center' style='color:Green;'><span class='glyphicon glyphicon-ok'></span></td>";                                     
             }        
            else  $strOutPut .="<td style='background-color:$bg'></td>";    
            */
    
            $id = "isExec".$timeList['eventRef'];                       
               $action = "switchData(".$timeList['eventRef'].", 'isExec');";                                  
               $val = \yii\helpers\Html::tag( 'div', '&nbsp;', 
                   [
                     'class'   => 'btn btn-primary btn-small',
                     'id'      => $id,
            ...
            $strOutPut .= "<td style='background-color:$bg;'><div style='font-size:10pt;' title='".$freeTimeList[$iRow]['execNote']."'>";
            $strOutPut .= mb_substr($freeTimeList[$iRow]['execNote'],0,220,'utf-8').$add."</div></td>";    
    
        }
        else //пустая строка
        {        
            $strOutPut .="<td style='background-color:$bg'><div class='freetime' ";
