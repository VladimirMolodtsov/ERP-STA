...
$N = count($freeTimeList)-$range;
   ...
   echo "<tr>\n";    
    ...
    //echo "<td> $iRow $eRow ".date("H:i", $curTime-$model::TIMESHIFT)."</td>";
    
    $rowSTime=$freeTimeList[$iRow]['time']+$model::TIMESHIFT;
    $rowETime=$freeTimeList[$eRow]['time']+$model::TIMESHIFT;
    if ($curTime > $rowSTime && $curTime <= $rowETime)
    {
    ...
    "</td>";
      ...
      */      
      ...
      elseif ($curTime >  $rowETime) $curTimeStatus=2; // просрочено сегодня
      elseif ($curTime >  $rowSTime) $curTimeStatus=1; // в ходе исполнения
