...
if ($model->managerCount ==0) return;
   ...
   for ($iRow=0;$iRow<62;$iRow+=3)
    {
    echo "<tr>\n";    
    
    $bg="";
    if ($curTime > $freeTimeList[0]['timeList'][$iRow]['time'] && $curTime <= $freeTimeList[0]['timeList'][$iRow+3]['time'])
    {
     $bg="style='background-color:Orange;'";    
    }
    echo "<td align='center' $bg> ".$freeTimeList[0]['timeList'][$iRow]['strTime']." - ".$freeTimeList[0]['timeList'][$iRow+3]['strTime']." <br>".    
    //date("H:i",$freeTimeList[0]['timeList'][$iRow]['time'])." ".date("H:i",$freeTimeList[0]['timeList'][$iRow+3]['time'])."<br>".
    "</td>";
    
    for ($iCol=0;$iCol<$model->managerCount;$iCol++)
    {
      $timeList[0]=$freeTimeList[$iCol]['timeList'][$iRow];
      $timeList[1]=$freeTimeList[$iCol]['timeList'][$iRow+1];
      $timeList[2]=$freeTimeList[$iCol]['timeList'][$iRow+2];
      ...
      */      
      ...
      elseif ($curTime >  $timeList[2]['time']) $curTimeStatus=2; // просрочено сегодня
      elseif ($curTime >  $timeList[0]['time']) $curTimeStatus=1; // в ходе исполнения
       ...
       for ($i=0; $i< 3; $i++) 
       {
        if($timeList[$i]['eventRef'] == 0) continue;        
          ...
          else $strDuty .=$timeList[$i]['orgTitle']."\n";        
          $strAll .=$timeList[$i]['orgTitle']."\n";                         
       }
