...
function printZakaz (mode)
{
    var url = 'market/print-zakaz&zakazId=<?=$model->zakazId?>&mode='+mode;
...
function createLead()
{
  var orgId = $('#orgId').val();
  if (orgId == 0) {
    alert ("Операция невозможна - организация не задана!");
    return;
  }
  
  var URL = 'index.php?r=market/create-lead&zakazId=<?=$zakazRecord['id']?>';
  console.log(URL); 
    $.ajax({
        url: URL,
        type: 'GET',
...
//        data: data,
        success: function(res){     
            if(res.res == true){
                openWin('site/lead-process&contactId='+res.val,'contactWin');        
  ...
  <table border='0' width='100%'>
   ...
   <?= \yii\helpers\Html::tag( 'div', $model->docList, 
    ...
    <td width='80px' valign='top' align='right'>
      <div style='margin-bottom:5px; margin-righht:10px;margin-right:5px;'>
           <?php 
                 echo \yii\helpers\Html::tag( 'div', "КП", 
                   [
                     'class'   => 'btn btn-default',
                     'id'      => 'print',
                     'onclick' => 'printZakaz (1);',
...
/*    $id = 'btnTarif';
    ...
    echo $contentWare;               
        ...
        <div style='display:inline; float:right'>
          Сумма по товарам: <b><?= number_format($model->sumZakaz,2,'.','&nbsp;') ?> руб. &nbsp;&nbsp;</b>
        </div>
     </div>   
  </div>
<!---->
...
*/
    ...
    echo \yii\helpers\Html::tag( 'div', 'Межгород', 
      ...
      <div style='display:inline; float:right'>
          Сумма по доставке: <b><?= number_format($model->sumTransport,2,'.','&nbsp;') ?> руб. &nbsp;&nbsp;</b>
        </div>
     </div>   
  </div>

  <div style='width:100%;font-weight:bold;font-size:15pt;color:Brown;text-align:right'>
          ИТОГО: <b><?= number_format($model->sumTransport+$model->sumZakaz,2,'.','&nbsp;') ?> руб. &nbsp;&nbsp;</b>
  </div>
</td>
...
<tr>
  <td  width='400px'>
    <p>Исходная заявка</p>
       <?php
       if (empty($model->initLeadRef)) $action="createLead();";
