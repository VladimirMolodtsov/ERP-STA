...
<tr>
  <td width='350px' valign='top'> <?php/*левый блок*/?>  
    <div class="page-title"><?= Html::encode($this->title) ?>  № 
        ...
        onclick="javascript:openWin('index.php?r=market/market-zakaz&noframe=1&parent=schet&orgId=<?= $record->id?>&zakazId=<?= $model->zakazId?>', 'zakazWin');"/>
...
if ($model->isFinishSupplyRequest == 1) $action= "javascript:chngSupplyStatus(".$last.");";       
  ...
  <div>   
    ...
    <td>Телефон:</td>
    <td><div style="position:relative;left:-80px"><?= $form->field($model, 'contactPhone')->textInput(['style'=>'width:300px; margin:0px; padding:10px; left:0px'])->label(false)?></div></td>
    <td valign='top'> <a href='#' class="btn btn-primary" title="Позвонить" onclick="javascript:doCall();"/><span class="glyphicon glyphicon-phone-alt"></span></a></td>
    </tr><tr>    
    <td>E-Mail:</td>
