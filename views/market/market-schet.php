...
function addAct()
{
 ...
 var URL = 'index.php?r=/market/add-act-in-schet&schetId=<?=$schetRecord->id?>';
  console.log(URL); 
    $.ajax({
        url: URL,
        type: 'GET',
...
<tr>
  ...
  <?php
     ...
     $actList=$model->getActList ();
     $N=count($actList);
    for ($i=0;$i<$N;$i++){
    $action = "openWin('market/market-akt&id=".$actList[$i]['id']."','actWin')";
         echo \yii\helpers\Html::tag( 'div', $actList[$i]['actNum'], 
         [
                     'class'   => 'clickable',
                     'onclick' => $action,
                     'title'   => 'акт сверки'     
