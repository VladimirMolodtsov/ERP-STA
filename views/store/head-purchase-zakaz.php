...
$this->title = 'Создать запрос цены и расчет потребности';
if ($model->id > 0)  
{ 
    $zaprosRecord = $model->preparePurchaseZakaz();                  
    //print_r($zaprosRecord);
...
function regRequest(requestId, orgRef)
{ 

 var  url = 'site/reg-contact&id='+orgRef+'&purchaseRef=<?= $model->id ?>&purchaseVar='+requestId;;
 openWin(url, 'contactWin');
   
 document.forms["RequestForm"]["editVariantId"].value=requestId;
 //document.forms["RequestForm"]["editRequestDate"].value=;
 var url = 'index.php?r=store/purchase-set-request&id=<?= $model->id ?>&editVariantId='+requestId;
 console.log(url);
 $.ajax({
 url: url,
 type: 'GET',
...
<br>
<?= $form->field($model, 'zakazNote')->textarea(['id' => 'zakazNote','row' =>5, 'style'=>'margin:0px; padding:0px; height:175px; left:0px'])->label('Комментарий к запросу')?>
<?= $form->field($model, 'id')->hiddenInput()->label(false)?> 
<br>
<div style='text-align: right;'>
...
<div class='row'>
    <div class='col-md-3'>
        <input <?=$disChange?>  class="btn btn-primary"  style="width: 170px;" type="button" value="Добавить поставщика" onclick='showDialog("#org_list_form");' />
    </div>
    <div class='col-md-3'>
    ...
    <div class='col-md-3'>
    <?php if($model->showList['isAgreed'] == 1){ ?>    
        <input  class="btn btn-primary"  style="width: 170px;" type="button" value="Отклонить" onclick='setDenied();' />                
     <?php } ?>   
    </div>
    
    <div class='col-md-3'>    
    <?php if($model->showList['isAgreed']==1){ ?>    
        <input  class="btn btn-primary"  style="width: 170px;" type="button" value="Согласовать" onclick='setPermited();' />            
    <?php } ?>
    <?php if($model->showList['isAgreed']==2){ ?>    
        <input  class="btn btn-primary"  style="width: 200px;" type="button" value="Отозвать согласование" onclick='setUnPermited();' />            
    <?php } ?>
    </div>
</div>
<?php }?>
...
<div class='row'>
    <div class='col-md-3'>
        <input class="btn btn-primary"  style="width: 200px;" type="button" value="Добавить поставщика" onclick='showDialog("#org_list_form");' />
    </div>
    ...
    <?php if ($zaprosRecord->status == 0){ ?>    
        <input  class="btn btn-primary"  style="width: 200px;" type="button" value="Одобрить" onclick='setInWork();' />            
    <?php } ?>
...
//		'filterModel' => $goodModel,
        ...
        'columns' => [
            ...
            [
                ...
                $ret = "<div class='gridcell' onclick='javascript:regRequest(".$model['id'].",".$model['refOrg'].")'>".$lbl."</div>";
                if ($isFlg ) $ret.="<br>".$model['requestDate'];
				return $ret;
                },               
			],
...
{ 
	...
	{ // ловим клик по крестику или подложке
		$('#good_list_form', '#org_list_form', '#request_form', '#schet_list_form')
