...
function setVertical()
{
    ...
    window.resizeTo(710, screenHeight-70);
    
    window.moveTo((screenWidth-800), 0);
    //window.moveTo(0, 0);
    document.location.href='index.php?r=site/org-card&orgId=<?= $model->orgId ?>&noframe=1&viewmode=v';
}
...
<div class='row'>
    <div class='col-md-4' style='margin-top:10px; font-size:15pt;'>
    ...
    <?php
     ...
     <?php
        $query = preg_replace("/\s+/","+",$model->title);
        $action= "setVertical();";
        echo \yii\helpers\Html::tag( 'div', "Вертикальный вид",
                   [
                     'class'   => 'btn btn-default',
                     'onclick' => $action,
...
<table border=0 width=100% cellpadding="5px" style="padding:5px">
  <tr>
      ...
      <td>Взаимодействие с предприятием</td>
      <td> 
      <?php
      if (mb_strlen($model->orgDealTitle,'utf-8') > 25) $style="font-size:11px;";
      else $style ='';
