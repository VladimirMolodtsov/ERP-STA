...
class EventRegForm extends Model
{
   ...
   public $managerCount=0;
    ...
    public function getFreeTimeList($strDate, $userId)
    {
        ...
        eventNote, eventStatus, preevent.note as preNote,postcontact.note as postNote, 
        refExecute, executeDateTime, {{%calendar}}.ref_contact 
        from {{%calendar}}         
    ...
    //  print_r($usedTime);
        ...
        for ($i=0; $i <$N; $i++) 
        {
          ...
          $idx = intval( ($curTime - $startTime)/300 );
     //     echo $idx."\n";
  ...
  {
        $list = Yii::$app->db->createCommand("Select {{%tasks}}.id, {{%tasks}}.note, planDate, {{%orglist}}.title as orgTitle, creatorRef
